...
The firmware does not have a native CMake codebase, but you can use CMake as the build driver for Arduino CLI. The wrapper lives in [software/firmware/source/SoftRF/CMakeLists.txt](/run/media/decimal/dev/Projects/SoftRF/software/firmware/source/SoftRF/CMakeLists.txt) and builds the `SoftRF.ino` sketch with `arduino-cli`.

### Prerequisites

Install the required Arduino ESP32 core version (2.0.9 covers most targets):
...
Build for ESP32-C3 with huge_app partition scheme:

```sh
cmake -S software/firmware/source/SoftRF \
    -B /tmp/softrf-c3-build \
    -DSOFTRF_FQBN=esp32:esp32:esp32c3 \
    -DSOFTRF_BOARD_OPTIONS="PartitionScheme=huge_app"
cmake --build /tmp/softrf-c3-build
```

For ESP32 (generic Dev Module):

```sh
cmake -S software/firmware/source/SoftRF \
    -B /tmp/softrf-esp32-build \
    -DSOFTRF_FQBN=esp32:esp32:esp32 \
    -DSOFTRF_BOARD_OPTIONS="PartitionScheme=huge_app"
cmake --build /tmp/softrf-esp32-build
```

### Build Output

Compiled binaries are exported to `${BUILD_DIR}/binaries/` and include:

- **`SoftRF.ino.bin`** — Main firmware image (use this for OTA updates or web flashing)
- **`SoftRF.ino.bootloader.bin`** — Bootloader (rarely needed; flashed at 0x1000)
- **`SoftRF.ino.partitions.bin`** — Partition table (rarely needed; flashed at 0x8000)
- **`SoftRF.ino.elf`** — Debugging symbols; use with debugger or `esptool.py`
- **`SoftRF.ino.map`** — Link map; useful for analyzing memory layout

**To flash using esptool.py:**

```sh
esptool.py --chip esp32 --port /dev/ttyUSB0 --baud 921600 write_flash \
    0x1000 /tmp/softrf-c3-build/binaries/SoftRF.ino.bootloader.bin \
    0x8000 /tmp/softrf-c3-build/binaries/SoftRF.ino.partitions.bin \
    0x10000 /tmp/softrf-c3-build/binaries/SoftRF.ino.bin
```

### Multi-Target Build

Use the provided build script to compile for all supported targets:
...
```
~/softrf-binaries/
├── build-esp32/binaries/        # ESP32 Dev Module
├── build-esp32c3/binaries/      # ESP32-C3 Dev Module
├── build-esp32s3/binaries/      # ESP32-S3 Dev Module
├── build-esp32c6/binaries/      # ESP32-C6 Dev Module
└── build-esp32s2/binaries/      # ESP32-S2 Dev Module
```

To edit the list of targets, modify the `TARGETS` array in [software/firmware/build-all-targets.sh](/run/media/decimal/dev/Projects/SoftRF/software/firmware/build-all-targets.sh).

### CMake Configuration Options

| Variable               | Default                             | Description                                                            |
| ---------------------- | ----------------------------------- | ---------------------------------------------------------------------- |
| `SOFTRF_FQBN`          | `esp32:esp32:esp32`                 | Fully-qualified board name (FQBN) for arduino-cli                      |
| `SOFTRF_BOARD_OPTIONS` | `PartitionScheme=huge_app`          | Board options (see `arduino-cli board details -b <FQBN>`)              |
| `SOFTRF_PYTHONPATH`    | ``                                  | Path to Python site-packages (needed if pyserial not in system Python) |
| `SOFTRF_OUTPUT_DIR`    | `${CMAKE_BINARY_DIR}/binaries`      | Directory for exported binaries                                        |
| `SOFTRF_BUILD_DIR`     | `${CMAKE_BINARY_DIR}/arduino-build` | Temporary Arduino CLI build directory                                  |

Example with custom output directory:

```sh
cmake -S software/firmware/source/SoftRF -B /tmp/build \
    -DSOFTRF_FQBN=esp32:esp32:esp32c3 \
    -DSOFTRF_OUTPUT_DIR=/opt/softrf-binaries/c3
cmake --build /tmp/build
```

### Supported ESP32 Targets

| Board               | FQBN                              | Min Core    | Partition Schemes                          |
| ------------------- | --------------------------------- | ----------- | ------------------------------------------ |
| ESP32 Dev Module    | `esp32:esp32:esp32`               | 2.0.9       | `default`, `huge_app`, `no_ota`, `minimal` |
| ESP32-C3 Dev Module | `esp32:esp32:esp32c3`             | 2.0.9       | `default`, `huge_app`, `no_ota`, `minimal` |
| ESP32-S3 Dev Module | `esp32:esp32:esp32s3`             | 2.0.9       | `default`, `huge_app`, `no_ota`, `minimal` |
| ESP32-C6 Dev Module | `esp32:esp32:esp32c6`             | 3.0.7       | (same as above)                            |
| DOIT DevKit v1      | `esp32:esp32:esp32doit-devkit-v1` | 1.0.5–2.0.x | `default`, `no_ota`                        |

For C6, install core 3.0.7: `arduino-cli core install esp32:esp32@3.0.7`
